...
public class Index implements Serializable, Closeable {
    ...
    public void test( CarPark park ) throws KeyNotUniqueException {
        assert( park != null );
        if ( numbers.contains(park.number)) {
            throw new KeyNotUniqueException( park.number );
        }	
    }
	
    public void put( CarPark park, long value ) throws KeyNotUniqueException {
        test( park );
        numbers.put( park.number, value );
        names.put( park.name, value);	
        times.put(park.start_time, value);
    }
